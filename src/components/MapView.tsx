...
const MapView = ({ incidents }: MapViewProps) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const home = locationSettings.homeLocation;
      ...
      const x = (home.coordinates.lng - seoulCenter.lng) * 1000 + 50 + "%";
      const y = (-home.coordinates.lat + seoulCenter.lat) * 1000 + 50 + "%";
      marker.style.left = x;
      marker.style.top = y;
      marker.innerHTML = "🏠";
      marker.title = `내 집: ${home.name}`;
    ...
    incidents.forEach((incident, index) => {
      if (incident.coordinates) {
        ...
        // 상대적 위치 계산 (실제로는 지도 API의 projection 사용)
        const x =
          (incident.coordinates.lng - seoulCenter.lng) * 1000 + 50 + "%";
        const y =
          (-incident.coordinates.lat + seoulCenter.lat) * 1000 + 50 + "%";
